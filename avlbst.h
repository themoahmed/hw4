...
protected:
    ...
    void removeFix(AVLNode<Key, Value>* parent, AVLNode<Key, Value>* newNode);
...
void AVLTree<Key, Value>:: remove(const Key& key)
{
    ...
    if(RemoveNode->getLeft() == nullptr && RemoveNode->getRight() == nullptr){
        ...
        delete RemoveNode;
        removeFix(parent, nullptr);
    }
    else if(RemoveNode->getLeft() == nullptr || RemoveNode->getRight() == nullptr){
        AVLNode<Key, Value>* parent = RemoveNode->getParent();
        AVLNode<Key, Value>* child = RemoveNode->getLeft() == nullptr ? RemoveNode->getRight() : RemoveNode->getLeft();
        if(parent->getLeft() == RemoveNode){
        ...
        nodeSwap(RemoveNode, predecessor);
        AVLNode<Key, Value>* parent = RemoveNode->getParent();
        AVLNode<Key, Value>* child = RemoveNode->getLeft() == nullptr ? RemoveNode->getRight() : RemoveNode->getLeft();
        if(parent->getLeft() == RemoveNode){
            parent->setLeft(child);
    ...
    if((RemoveNode->getRight() == NULL && RemoveNode->getLeft() != nullptr) || (RemoveNode->getRight() != NULL && RemoveNode->getLeft() == nullptr)){
        AVLNode<Key, Value>* parent = RemoveNode->getParent();
        AVLNode<Key, Value>* child = RemoveNode->getRight() == nullptr ? RemoveNode->getLeft() : RemoveNode->getRight();
        if(parent->getLeft() == RemoveNode){
            parent->setLeft(child);
        }
        else{
...
template<class Key, class Value>
void AVLTree<Key, Value>::removeFix(AVLNode<Key, Value>* parent, AVLNode<Key, Value>* newNode)
{
    if(parent == nullptr) return;
    if(parent->getBalance() == 0){
        if(parent->getLeft() == newNode) parent->updateBalance(1);
        else if(parent->getRight() == newNode) parent->updateBalance(-1);
        return;
    }
